...
for (tissue in tissue_order) {
  ...
  v1 <- volcano_plot(
    ...
    geom_label(df_labels = male_features,
               x = male_features$`Log2 PM/SM`,
               y = male_features$`Negative Log(PM/SM p-value)`,
               colorvar = male_features$male_feats); v1
  ggsave(v1, filename = paste0("figures/volcano_plots/", tissue, "_males.svg"),
         height = 5.5, width = 5)
  v2 <- volcano_plot(
    df = df_stats,
    ...
    geom_label(df_labels = female_features,
               x = female_features$`Log2 PF/SF`,
               y = female_features$`Negative Log(PF/SF p-value)`,
               colorvar = female_features$female_feats); v2
  ggsave(v2, filename = paste0("figures/volcano_plots/", tissue, "_females.svg"),
         height = 5.5, width = 5)
  v3 <- volcano_plot(
    df = df_stats,
    x = df_stats$`Log2 P/S`,
    ...
    geom_label(df_labels = all_features,
               x = all_features$`Log2 P/S`,
               y = all_features$`Negative Log(P/S p-value)`,
               colorvar = all_features$all_sig); v3
  ggsave(v3, filename = paste0("figures/volcano_plots/", tissue, "_all.svg"),
         height = 5.5, width = 5)
