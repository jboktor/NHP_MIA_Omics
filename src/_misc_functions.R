...
boxplot_loop_treatment <- function(significant_mets,
  ...
  for (analyte in significant_mets) {
    ...
    plot <-
      ...
      geom_signif(
        comparisons = comparisons.Treatment,
    ...
    output_label <- paste0(
      "figures/boxplots/", tissue, "/", tissue,
...
boxplot_loop_treatment_sex <- function(significant_mets,
  ...
  for (analyte in significant_mets) {
    ...
    plot <-
      ...
      geom_signif(
        comparisons = comparisons.Treatment.Male,
        annotations = Msigvalue, tip_length = 0
      ) +
      geom_signif(
        comparisons = comparisons.Treatment.Female,
...
clean.cols.y4 <- function(x) {
  colnames(x) <- gsub("_y4", "", colnames(x))
  x
}
