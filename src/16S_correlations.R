...
ps_df_stool <- ps_obj %>% subset_samples(tissue == "Stool") %>% prep16SAbundTable()
ps_df_stool %>% dim()
ps_df_jej <- ps_obj %>% subset_samples(tissue == "Jejunum") %>% prep16SAbundTable()
ps_df_jej %>% dim()
ps_df_ile <- ps_obj %>% subset_samples(tissue == "Ileum") %>% prep16SAbundTable()
ps_df_ile %>% dim()
...
microbe_metabolite_cors <-
  bind_rows(microbe_mets_stool %>% corr_FDR_16S, 
...
microbe_cytokine_cors <-
  bind_rows(microbe_cyt_stool_df, 
