...
phyloseq_rarefy <- function(ps) {
  ...
  df_rarefied <-
    vegan::rrarefy(df, sample = min(rowSums(df))) %>%
    t() %>%
    as.data.frame()
...
unifrac_plots <- function(obj_dist) {
  ...
  wu_plot <-
    ...
    scale_xsidey_discrete(guide = "none") +
    scale_ysidex_discrete(guide = "none") +
    theme_bw() +
...
lm_basic <- function(metadf, metric) {
  formula <-
    as.formula(paste(metric, "~", paste(
      c("gender", "treatment", "gender*treatment"),
      collapse = "+"
    )))
...
alpha_div_analysis <- function(df, info) {
  ...
  for (metric in alpha_metrics) {
    ...
    metric_lm <- lm_mod %>%
      tidy() %>%
      mutate(alpha_metric = metric, tissue = info)
...
corr_heatmap_16s_to_cyts <-
  function(corr.df,
    ...
    h1 <-
      corr.df.trim.ordered %>%
      ggplot(aes(
        x = asv_label_unique, y = feature_B_obj,
        fill = rho
      )) +
      geom_tile() +
      labs(fill = "Spearman's correlation") +
